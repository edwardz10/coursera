...
public final class PageRank {
    ...
    public static JavaPairRDD<Integer, Double> sparkPageRank(
        ...
        final JavaPairRDD<Integer, Double> newRanks =
                sites.join(ranks)
                        .flatMapToPair(kv -> {
                            final Website edges = kv._2()._1();
                            final Double currentRank = kv._2()._2();
                            final List<Tuple2<Integer, Double>> contribs = new LinkedList<>();


                            final Iterator<Integer> it = edges.edgeIterator();
                            while (it.hasNext()) {
                                contribs.add(new Tuple2<>(it.next(), currentRank/(double)edges.getNEdges()));
                            }
