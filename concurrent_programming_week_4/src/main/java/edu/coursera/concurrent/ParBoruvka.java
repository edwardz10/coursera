        ...
        for (int i = 0; i < threadCount; i++) {
            while (true) {
                try {
                    ...
                    System.err.println("Failed to join a thread: " + ie);
                }
            }
        }

        final ParComponent resultNode = result;
        if (resultNode != null) {
            totalWeight = resultNode.totalWeight;
            totalEdges = resultNode.totalEdges;
        }
    }
